
import { Link } from "react-router-dom";
import { FaCartPlus } from 'react-icons/fa';



const Navbar = () => {

    // const cartItem = localStorage.getItem()


  const menuOptions = (
    <>
      <li><Link to='/'>Home</Link></li>
      <li><Link to='/movies'>Movies</Link></li>
      

    </>
  )
  return (
    <div className="md:px-24 px-4 shadow-md bg-slate-700  text-white w-full fixed z-10  top-0 bg-opacity-40   ">
      <div className="navbar  ">
        <div className="navbar-start">
          <div className="dropdown">
            <label tabIndex={0} className="btn btn-ghost lg:hidden">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
            </label>
            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 p-2 text-black shadow bg-base-100 rounded-box w-52">
              {menuOptions}
            </ul>
          </div>
          <Link to='/' className=" font-bold text-2xl uppercase">Movie <span className="text-yellow-400">Verse</span></Link>
        </div>
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1">
            {menuOptions}
          </ul>
        </div>
        <div className="navbar-end">
         
         
              <Link to='/login' className="btn-primary"><FaCartPlus/>
              </Link>
            
        </div>
      </div>

    </div>
  );
};

export default Navbar;